...
class Post extends \yii\db\ActiveRecord
{
    ...
    public function get_image($class=''){
        $img = false;
        preg_match ('#<img.*?src=["\']*([\S]+)["\'].*?>#si', $this->content, $img);
        if (!$img){
            $img = '<img src="/images/article.png"  title="'. $this->title .'">';
        }else{
            $img = '<img src="' . $img[1] . '" class="'.$class.'" title="'. $this->title .'"/>';
        }
        return $img;
    }
    ...
    public function getRelated(){
        return Post::find()
            ->joinWith('tags')
            ->where(['tags.id' => ArrayHelper::getColumn($this->tags, 'id')])
            ->andWhere('post.id != :id', ['id' => $this->id])
            ->limit(4)
            ->all();
    }
